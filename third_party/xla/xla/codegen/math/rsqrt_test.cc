...
TEST(RsqrtTest, Name) {
  EXPECT_EQ(Intrinsic::Rsqrt::Name(Intrinsic::S(F32)), "xla.rsqrt.f32");
  EXPECT_EQ(Intrinsic::Rsqrt::Name(Intrinsic::V(F32, 4)), "xla.rsqrt.v4f32");
  EXPECT_EQ(Intrinsic::Rsqrt::Name(Intrinsic::V(F64, 8)), "xla.rsqrt.v8f64");
}
...
JitRunner CreateJitRunnerWithRsqrt(Intrinsic::Type type) {
  auto context = std::make_unique<llvm::LLVMContext>();
  auto module = std::make_unique<llvm::Module>("test_module", *context);
  llvm::Function* rsqrt_func =
      Intrinsic::Rsqrt::CreateDefinition(module.get(), type).value();
...
TEST(RsqrtTest, EmitRsqrtF32) {
  if (isX86()) {
    ...
    auto rsqrt = jit.GetScalarFn<float(float)>(Intrinsic::Rsqrt::Name(type));
