...
JitRunner CreateJitRunner(Intrinsic::Type type) {
  auto context = std::make_unique<llvm::LLVMContext>();
  auto module = std::make_unique<llvm::Module>("test_module", *context);
  llvm::Function* erf_func =
      Intrinsic::Erf::CreateDefinition(module.get(), type).value();
...
TEST(ErfTest, F32) {
  ...
  auto fn = runner.GetScalarFn<float(float)>(Intrinsic::Erf::Name(type));
