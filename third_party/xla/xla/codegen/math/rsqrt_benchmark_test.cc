...
JitRunner CreateJitRunnerWithRsqrt(Intrinsic::Type type) {
  ...
  CreateOneOverSqrt(*context, *module, Intrinsic::TypeToIrType(type, *context));
...
static void BM_RsqrtVectorized(benchmark::State& state) {
  ...
  Intrinsic::Type intrinsic_type = Intrinsic::V(type, num_elements);
  JitRunner jit = CreateJitRunnerWithRsqrt(intrinsic_type);
  std::string function_name = (function == kRsqrt)
                                  ? Intrinsic::Rsqrt::Name(intrinsic_type)
                                  : "one_over_sqrt";
