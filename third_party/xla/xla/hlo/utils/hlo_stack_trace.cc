...
void BuildStackTracesRecursive(
    const HloInstruction* instr, std::deque<std::string> current_path,
  ...
  for (const HloInstruction* caller : parent_comp->caller_instructions()) {
...
std::string FormatStackTraceBreakdown(
  ...
  for (const auto& [bytes, value] : buffers) {
    std::vector<std::vector<std::string>> all_traces;
    BuildStackTracesRecursive(value->instruction(), {}, all_traces);
