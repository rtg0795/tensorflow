...
class UserContextScope {
 public:
  ...
  explicit UserContextScope(const UserContextRef& context);
  ...
  // Returns the active `UserContextRef`. Returns `nullptr` if there is no
  // `UserContextRef` in the scope.
 ...
 private:
  ...
  // The current scope's `UserContext`. Only used for checking invariants.
  const UserContextRef* const context_;  // Not owned.
};
