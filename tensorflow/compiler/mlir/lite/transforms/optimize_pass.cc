...
static std::optional<std::pair<int, Type>> GetFcNumChannelsAndFilterType(
  ...
  // If filter is unranked, we can't check if it's quantized.
  // As a proxy, check the output of the FC op.
  if (mlir::isa<quant::QuantizedType>(fc_output_type.getElementType())) {
    return std::nullopt;
  }
